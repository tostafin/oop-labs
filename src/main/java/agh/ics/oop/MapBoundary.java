...
public class MapBoundary implements IPositionChangeObserver {
    ...
    public void positionChanged(Vector2d oldPosition, Vector2d newPosition) {
        ...
        for (Animal a : xAnimals) {
            if (a.getPosition() == oldPosition) animal = a;
        }
        if (animal != null) {
    ...
    public Vector2d getLowerLeft() {
        int xMin = Math.min(xAnimals.first().getPosition().x, xGrass.first().getPosition().x);
        int yMin = Math.min(yAnimals.first().getPosition().y, yGrass.first().getPosition().y);
        return new Vector2d(xMin, yMin);
    }

    public Vector2d getUpperRight() {
        int xMax = Math.max(xAnimals.last().getPosition().x, xGrass.last().getPosition().x);
        int yMax = Math.max(yAnimals.last().getPosition().y, yGrass.last().getPosition().y);
